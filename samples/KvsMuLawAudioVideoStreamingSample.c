...
#define NUMBER_OF_ULAW_FRAME_FILES          911
...
typedef struct {
    ...
    FrameData audioFrames[NUMBER_OF_ULAW_FRAME_FILES];
...
PVOID putAudioFrameRoutine(PVOID args)
{
    ...
    while (defaultGetTime() < data->streamStopTime) {
        ...
        if (ATOMIC_LOAD_BOOL(&data->firstVideoFramePut)) {
            ...
            fileIndex = (fileIndex + 1) % NUMBER_OF_ULAW_FRAME_FILES;
            frame.frameData = data->audioFrames[fileIndex].buffer;
            frame.size = data->audioFrames[fileIndex].size;
...
INT32 main(INT32 argc, CHAR *argv[])
{
    ...
    BYTE audioCpd[KVS_PCM_CPD_SIZE_BYTE];
    ...
    for(i = 0; i < NUMBER_OF_ULAW_FRAME_FILES; ++i) {
        SNPRINTF(filePath, MAX_PATH_LEN, "%s/pcmMuLawTrack1SampleFrames/%03d.ulaw", data.sampleDir, i + 1);
    ...
    // CHK_STATUS(createRealtimeMuLawAudioVideoStreamInfoProvider(streamName, DEFAULT_RETENTION_PERIOD, DEFAULT_BUFFER_DURATION, &pStreamInfo));
    CHK_STATUS(createRealtimeAudioVideoStreamInfoProviderWithCodecs(streamName, DEFAULT_RETENTION_PERIOD, DEFAULT_BUFFER_DURATION, VIDEO_CODEC_ID_H264,
                                                                        AUDIO_CODEC_ID_PCM_MULAW, &pStreamInfo));
    ...
    CHK_STATUS(mkvgenGeneratePcmCpd(KVS_PCM_FORMAT_CODE_MULAW, AUDIO_TRACK_SAMPLING_RATE, 1 /* mono */, pAudioTrack->codecPrivateData, pAudioTrack->codecPrivateDataSize));
