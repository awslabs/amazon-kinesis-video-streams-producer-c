...
STATUS continuousRetryStreamConnectionStaleHandler(UINT64 customData, STREAM_HANDLE streamHandle, UINT64 lastBufferingAck)
{
    ...
    CHK_STATUS(connectionStaleStateMachineHandleConnectionStale(
        streamHandle,
        &pCallbackStateMachine->connectionStaleStateMachine,
        pCallbackStateMachine->pExponentialBackoffState));
                                               ...
                                               STATUS statusCode)
{
    ...
    if (!(IS_RETRIABLE_PRODUCER_ERROR(statusCode) || IS_RETRIABLE_COMMON_LIB_ERROR(statusCode))) {
        ...
        //
        // From where is this errorReporterHandler called?
        //  (1) On Tag stream failure
        //  (2) On stream termination if we did not receive last persist ACK
        //  (3) On stream fragment error acknowledgement
        //  (4) Failures for CreateStream API, DescribeStream API, GetStreamEndpoint API, TagResouce API and PutStream API
        //
        // If a recoverable error, we let the SDK handle it. SDK in this case means the PIC layer.
        // A recoverable error, for example, could be timeouts on service API calls. For such errors,
        // we don't need to reset the stream. We can just retry. And hence we return from this point.
        //
        if (IS_RECOVERABLE_ERROR(statusCode)) {
            CHK_STATUS(getStreamMapping(pContinuousRetryStreamCallbacks, streamHandle, &pCallbackStateMachine));
        ...
        //  PIC's stepStateMachine API returns special error codes for service API call failures -
        //  DESCRIBE_STREAM_CALL, CREATE_STREAM_CALL, GET_STREAMING_TOKEN_CALL, GET_STREAMING_ENDPOINT_CALL,
        //  PUT_STREAM_CALL, INVALID_TOKEN_EXPIRATION
        //
        // For service API call failures, PIC internally retries SERVICE_CALL_MAX_RETRY_COUNT number of times
        // before returning these error codes.
        // If we get either of these return codes, that means we've exhausted PIC retries.
        // https://github.com/awslabs/amazon-kinesis-video-streams-pic/blob/9ef93e5a41465ec0c847b8e1ee30ac9ba29fdb7f/src/state/src/State.c#L168-L171
        // In that case, we need to reset the stream (proceed to continuousRetryStreamRestartHandler)
    ...
    // Inject wait time before resetting the stream
    CHK_STATUS(exponentialBackoffBlockingWait(pCallbackStateMachine->pExponentialBackoffState));
...
STATUS continuousRetryStreamLatencyPressureHandler(UINT64 customData, STREAM_HANDLE streamHandle, UINT64 currentBufferDuration)
{
    ...
    CHK_STATUS(getStreamMapping(pContinuousRetryStreamCallbacks, streamHandle, &pCallbackStateMachine));
    CHK_STATUS(streamLatencyStateMachineHandleStreamLatency(
        streamHandle,
        &pCallbackStateMachine->streamLatencyStateMachine,
        pCallbackStateMachine->pExponentialBackoffState));
