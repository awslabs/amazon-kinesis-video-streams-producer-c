...
STATUS createIotAuthCallbacksWithTimeoutsWithIpVersion(PClientCallbacks pCallbacksProvider, PCHAR iotGetCredentialEndpoint, PCHAR certPath, PCHAR privateKeyPath,
                                          PCHAR caCertPath, PCHAR roleAlias, PCHAR streamName, UINT64 connectionTimeout, UINT64 completionTimeout, IP_VERSION ipVersion,
                                          PAuthCallbacks* ppIotAuthCallbacks)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    ...
    CHK_STATUS(createCurlIotCredentialProviderWithTimeAndTimeoutAndIpVersion(
        iotGetCredentialEndpoint, certPath, privateKeyPath, caCertPath, roleAlias, streamName, connectionTimeout, completionTimeout,
        pIotAuthCallbacks->pCallbacksProvider->clientCallbacks.getCurrentTimeFn, ipVersion, pIotAuthCallbacks->pCallbacksProvider->clientCallbacks.customData,
        (PAwsCredentialProvider*) &pIotAuthCallbacks->pCredentialProvider));
